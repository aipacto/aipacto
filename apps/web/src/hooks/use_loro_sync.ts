...
// import { createLoroDocumentConnector } from '../lib/loro_document_connector'
// import { UniverLoroTransformer } from '../lib/univer_loro_transformer'
...
// 	debounceMs?: number
// 	enabled?: boolean // Add this to control whether sync should be active
// }
...
// 	const lastSnapshotRef = useRef<IDocumentData | null>(null)
...
// 					const univerDoc = UniverLoroTransformer.loroToUniver(loroDoc)
// 					lastSnapshotRef.current = univerDoc
// 				} else {
// 					// New document
// 					const loroDoc = new LoroDoc()
// 					loroDoc.setPeerId(Number(documentId))
...
// 	// Handle document change from Univer
// 	const handleDocumentChange = useCallback(
// 		(documentData: IDocumentData) => {
// 			if (!loroDocRef.current || !enabled || !authToken) return

// 			try {
// 				// Convert Univer document to Loro
// 				const newLoroDoc = UniverLoroTransformer.univerToLoro(documentData)

// 				// Get the update between old and new
// 				const oldVersion = loroDocRef.current.version()
// 				loroDocRef.current = newLoroDoc
...
// 					syncDocument(oldVersion)
// 				}, debounceMs)

// 				lastSnapshotRef.current = documentData
// 			} catch (error) {
// 				onError?.(error as Error)
// 			}
// 		},
...
// 		async (selection: ISetTextSelectionsOperationParams) => {
...
// 					// Convert back to Univer format
// 					const univerDoc = UniverLoroTransformer.loroToUniver(
// 						loroDocRef.current,
// 					)
// 					lastSnapshotRef.current = univerDoc
...
// 	// Get current document as Univer format
// 	const getCurrentDocument = useCallback((): IDocumentData | null => {
// 		if (!loroDocRef.current || !state.isInitialized) return null
// 		return UniverLoroTransformer.loroToUniver(loroDocRef.current)
// 	}, [state.isInitialized])
